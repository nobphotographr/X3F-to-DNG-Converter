...
pub fn write(reader: &Reader, path: impl AsRef<Path>, opts: &ProcessOptions) -> Result<(), Error> {
    ...
    write_to(reader, writer, opts, path).map(|_| ())
}

/// Render a DNG into an in-memory byte buffer.
///
/// This is the browser-WASM counterpart of [`write`]. It runs the exact same
/// conversion and TIFF/DNG writer, but uses a seekable memory cursor instead
/// of a host filesystem path.
pub fn write_vec(reader: &Reader, opts: &ProcessOptions) -> Result<Vec<u8>, Error> {
    let cursor = Cursor::new(Vec::new());
    let cursor = write_to(reader, cursor, opts, Path::new("<memory>"))?;
...
fn write_to<W: Write + Seek>(
    ...
    let mut tiff = TiffWriter::new(out).map_err(io_err(path))?;
