...
impl Reader {
    ...
    /// Render a Linear DNG into memory without touching a host filesystem.
    /// Intended for browser WebAssembly and other byte-buffer consumers.
    pub fn dump_dng_to_vec(&self, opts: &ProcessOptions) -> Result<Vec<u8>, Error> {
        output::dng::write_vec(self, opts)
    }
